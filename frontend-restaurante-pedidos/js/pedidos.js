...
controller.update_pedido = (req, res) => {
    ...
    // Validar que el ID esté presente (antes no se validaba y la query fallaba en silencio)
    if (!id) {
        return res.status(400).json({
            success: false,
    ...
    req.getConnection((err, conn) => {
        ...
        // "id" viaja en el body para identificar el pedido a actualizar, por eso
        // se incluye en validKeys; antes se rechazaba cualquier petición que lo trajera.
        ...
        conn.query(query, params, (err, result) => {
            if (err) {
                console.log(err);
                // Antes: res.send('Error al actualizar el pedido') (texto plano, rompía el
                // response.json() del frontend). Ahora responde JSON como el resto de la API.
                return res.status(500).json({
                    success: false,
            ...
            // Antes: res.send("Pedido actualizado con éxito") (texto plano).
            res.status(200).json({
                success: true,
